"""Tests for ``lose-it login`` populating the YAML config file.
...
from lose_it_utils.client import auth as auth_module
from lose_it_utils.client._settings import write_yaml_config
...
def test_login_writes_config_from_jwt_email(
    ...
    monkeypatch.setattr("lose_it_utils.cli.refresh_token_from_browser", lambda _b: jwt)
...
def test_login_uses_cookie_when_jwt_has_no_email(
    ...
    monkeypatch.setattr("lose_it_utils.cli.refresh_token_from_browser", lambda _b: jwt)
    monkeypatch.setattr(
        "lose_it_utils.cli.load_cookies_from_browser",
...
def test_json_mode_with_unresolvable_user_name_skips_yaml(
    ...
    monkeypatch.setattr("lose_it_utils.cli.refresh_token_from_browser", lambda _b: jwt)
    monkeypatch.setattr("lose_it_utils.cli.load_cookies_from_browser", lambda _b: {})
...
def test_login_preserves_unrelated_yaml_keys(
    ...
    """A user-set policy_hash mustn't be wiped out by `lose-it login`."""
    config_file = tmp_path / "config.yaml"
