...
def _build_log_payload(
    ...
    parts += [
        "18", "0",
        "19", "20", day_key, str(day_num), str(config.hours_from_gmt),
        "0", "-1", "-1", "0", "0", "0",
        "21", str(meal_ordinal), "0",
        "22", "23",
        "1", servings_str,
        "24", str(len(nutrients)),
    ]
...
def log_food(
    ...
    http.post_rpc(_build_log_payload(
        http.config, unsaved, meal_ordinal, day_key, day_num, servings,
    ))
...
def _build_delete_payload(config: Config, entry: FoodLogEntry) -> str:
    ...
    parts += [
        ...
        "0", "-1", "1", "0", "0", "0",
        "19", str(entry.meal_ordinal), "20", str(entry.extra_ordinal),
        "21", "22", servings_str, servings_str, "23", str(len(entry.nutrients_ordered)),
    ]
    for ord_, val in entry.nutrients_ordered:
        parts += ["24", str(int(ord_)), "25", fmt_num(val)]
    parts += [
        "26", servings_str, "0", "27", str(entry.food_measure_ordinal),
